<?php
	...
	function getCube()
	{
		...
		$query = 
		"SELECT 
			product.*, promotion.*, time.*, SUM(salesfact.unit_sales), ROUND(SUM(salesfact.dollar_sales), 2), ROUND(SUM(salesfact.dollar_cost), 2), SUM(salesfact.customer_count)
		FROM 
			product, promotion, time, salesfact 
		WHERE 
			product.product_key = salesfact.product_key AND promotion.promotion_key = salesfact.promotion_key AND time.time_key = salesfact.time_key 
		GROUP BY 
			product.description, promotion.promotion_name, time.date;";

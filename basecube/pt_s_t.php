<?php
	...
	function getCube()
	{
		...
		$query = 
		"SELECT 
			product.*, store.*, time.*, SUM(salesfact.unit_sales), ROUND(SUM(salesfact.dollar_sales), 2), ROUND(SUM(salesfact.dollar_cost), 2), SUM(salesfact.customer_count)
		FROM 
			product, store, time, salesfact 
		WHERE 
			product.product_key = salesfact.product_key AND store.store_key = salesfact.store_key AND time.time_key = salesfact.time_key 
		GROUP BY 
			product.description, store.name, time.date;";

<?php
	...
	function getCube()
	{
		...
		$query = 
		"SELECT 
			promotion.*, time.*, SUM(salesfact.unit_sales), ROUND(SUM(salesfact.dollar_sales), 2), ROUND(SUM(salesfact.dollar_cost), 2), SUM(salesfact.customer_count)
		FROM 
			promotion, time, salesfact 
		WHERE 
			promotion.promotion_key = salesfact.promotion_key AND time.time_key = salesfact.time_key 
		GROUP BY 
			promotion.promotion_name, time.date;";

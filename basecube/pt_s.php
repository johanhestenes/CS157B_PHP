<?php
	...
	function getCube()
	{
		...
		$query = 
		"SELECT 
			product.*, store.*, SUM(salesfact.unit_sales), ROUND(SUM(salesfact.dollar_sales), 2), ROUND(SUM(salesfact.dollar_cost), 2), SUM(salesfact.customer_count)
		FROM 
			product, store, salesfact 
		WHERE 
			product.product_key = salesfact.product_key AND store.store_key = salesfact.store_key
		GROUP BY 
			product.description, store.name;";
